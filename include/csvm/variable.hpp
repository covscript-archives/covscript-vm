...
namespace cs {
    ...
    struct select_if<A, B, true> {
        template <typename T> using result = A<T>;
    };
    template <template <typename> class A,
            template <typename> class B>
    struct select_if<A, B, false> {
        template <typename T> using result = B<T>;
    };
    ...
    class var_t final {
        ...
        template <typename T> using data_impl = typename select_if<data_impl_copyable, data_impl_noncopyable, is_copyable<T>::result>::template result<T>;
        ...
        struct data_store final {
            ...
            template <typename T> using sso_dispatcher = typename select_if<sso_enabled_policy, sso_disabled_policy,
                    sizeof(data_impl<T>) <= sso_threshold>::template result<T>;
